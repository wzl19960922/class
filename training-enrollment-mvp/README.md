...
### PowerShell 重定向说明（如果你在其他项目导入 MySQL schema）

PowerShell 不支持直接使用 `mysql -u root -p < scripts/schema.sql` 这种 Bash 重定向写法。
可改为以下任一方式：

```powershell
Get-Content .\scripts\schema.sql | mysql -u root -p
```
